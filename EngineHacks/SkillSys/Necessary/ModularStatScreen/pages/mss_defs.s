...
.macro draw_skillname_at tile_x, tile_y, skillNum=0, width=14, colour=3, growth_func=-1		@growth func is # of growth getter in growth_getters_table; 0=hp, 1=str, 2=skl, etc
  @Get skills in buffer
  mov r0, r8
  ldr r1, =Skill_Getter
  mov lr, r1
  .short 0xf800 @skills now stored in the skills buffer
  @Get the skillID for the specified skill number
  mov r6, r0
  ldrb r0, [r6, #\skillNum] 
  cmp r0, #0
  beq SkillEnd
  ...
  ldrh r0, [r3, r2]
  @Beneath this is the original version of draw_skillname_at, super strongly resembling draw_textID_at
  mov r3, r7
  mov r1, #\width
  @r3 is current buffer location, r1 is width.
  ldrh r2,[r3] @current number
...
.macro draw_status_text_at, tile_x, tile_y, colour=Blue  
  ...
  cmp r2, #0
  beq NoStatusCount
  ldr r0, =(0x2003ca2+(0x20*2*\tile_y)+(2*\tile_x+4))
  lsr r2, #4
  mov r1, #0
  blh 0x8004be4
  NoStatusCount:
